...
static Object *Page_Black_List( struct Config *cfg UNUSED )
{
Object *o;
...
//				LISTBROWSER_Labels,				& MainListHeader,
			...
			LAYOUT_AddChild,					IIntuition->NewObject( LayoutClass, NULL,
				...
				LAYOUT_AddChild,				IIntuition->NewObject( LayoutClass, NULL,
					...
					LAYOUT_AddChild,			GUIObjects[ GID_BlackAdd ] = IIntuition->NewObject( ButtonClass, NULL,
						GA_ID,					GID_BlackAdd,
						GA_Text,				"Add",
						GA_RelVerify,			TRUE,
					End,
...
int len;
	...
	IListBrowser->FreeListBrowserList( & MainBlackList );
...
static void myBlackEdit( struct Config *cfg, struct Node *node )
{
	if ( ! node )
	{
		return;
...
uint32 pos;
	...
	{
		myBlackEdit( cfg, node );
	}
}

// --

static void GUIFunc_BlackEdit( struct Config *cfg )
{
struct Node *node;
...
static void GUIFunc_BlackAdd( struct Config *cfg )
{
	if ( cfg->cfg_IPEditStat != IPES_Unused )
	{
		return;
...
static void GUIFunc_BlackDelete( struct Config *cfg )
{
struct IPNode *node;
struct Node *n;
