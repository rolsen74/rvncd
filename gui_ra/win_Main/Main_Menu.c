...
static void myMenu_User1_Kick( struct Config *cfg )
{
	if ( cfg->UserCount )
	{
		...
		{
			cfg->cfg_NetReason = mem_ASPrintF( "Kicking User 1" );
		}
...
//S32 err;
	...
	if ( ! AslHandle )
	{
		AslHandle = AllocAslRequestTags( ASL_FileRequest,
	...
	if ( stat )
	{
		...
		if (( AslHandle->fr_Drawer ) && ( AslHandle->fr_Drawer[0] ))
		{
			...
			{
				Filename = mem_ASPrintF( "%s%s", AslHandle->fr_Drawer, AslHandle->fr_File );
			}
			else
			{
				Filename = mem_ASPrintF( "%s/%s", AslHandle->fr_Drawer, AslHandle->fr_File );
		...
		{
			Filename = mem_ASPrintF( "%s", AslHandle->fr_File );
		}
...
S32 err;
	...
	if ( stat )
	{
		...
		if ( err )
		{
			Log_PrintF( cfg, LOGTYPE_Error, "Failed to load '%s' Config file", FilePart( Filename ));
		}
		else
		{
			if ( ActiveGUI.Refresh_Settings )
			...
			Log_PrintF( cfg, LOGTYPE_Info, "Config file '%s' loaded", FilePart( Filename ) );
		}
	}
...
S32 myGUI_MenuID( struct Config *cfg, U32 id )
{
S32 retval;
	...
	switch( id )
	{
		case MENUID_KeyLogger:
		{
			...
			if ( cfg->cfg_WinData[WIN_KeyLogger].OpenWin )
			{
		...
		case MENUID_Encodings:
		{
			...
			if ( cfg->cfg_WinData[WIN_Encodings].OpenWin )
			{
		...
		case MENUID_PixelFormat:
		{
			...
			if ( cfg->cfg_WinData[WIN_PixelFormat].OpenWin )
			{
		...
		case MENUID_Session:
		{
			...
			if ( cfg->cfg_WinData[WIN_Session].OpenWin )
			{
		...
		case MENUID_About:
		{
			...
			if ( cfg->cfg_WinData[WIN_About].OpenWin )
			{
...
S32 retval;
	...
	while( TRUE )
	{
		id = IDoMethod( MainMenuStrip, MM_NEXTSELECT, 0, id );
...
//		m = (PTR) IDoMethod( MainMenuStrip, MM_FINDID, 0, id );
...
void myGUI_DeleteMenus( struct Config *cfg UNUSED )
{
	if ( MainMenuStrip )
	{
		DisposeObject( MainMenuStrip );
		MainMenuStrip = NULL;
	}
}
