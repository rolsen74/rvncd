...
//	ISocket = cfg->NetRead_ISocket;
	...
	rc = Func_NetRead( cfg, km, size, MSG_WAITALL );
...
//			cfg->NetRead_Qualifier |= IEQUALIFIER_NUMERICPAD;
		...
		case 0xff9e: // Keypad List_Insert
		{
			cfg->NetRead_Qualifier |= IEQUALIFIER_NUMERICPAD;
			text = "Keypad List_Insert";
			code = 0x0f;
			break;
		}
...
// DebugPrintF( "Key Event: 0x%08lx\n", km->km_Key );
			...
			if ( MapANSI( (PTR) rawkey, strlen( rawkey ), buf, sizeof( buf ), NULL ) <= 0 )
			{
				discarded = TRUE;
			}
...

{
	...
	msg = mem_AllocClr( sizeof( struct CommandKey ) );
	...
	if ( msg )
	{
		...
		if ( ! discarded )
		{
			snprintf( msg->ck_Client, 64, "Amiga: $%02X, Client: $%04" PRIX32 " : %s : %s",
				code,
				km->km_Key, 
				( km->km_Down ) ? "Down" : "Up  ",
				text
		...
		{
			snprintf( msg->ck_Client, 64, "Amiga: $--, Client: $%04" PRIX32 " : %s - %s",
				km->km_Key, 
				( km->km_Down ) ? "Down" : "Up",
				text
			);
		...
		MsgPort_PutMsg( & CmdMsgPort, & msg->ck_Message );
	}
}
