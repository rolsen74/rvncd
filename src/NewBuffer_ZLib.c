...
S32 myEnc_ZLib( struct Config *cfg, struct UpdateNode *un UNUSED, U32 tilenr )
{
S32 datalen;
	...
	{
		datalen = myEnc_ZLib_8( cfg, un, tilenr );
	}
	else if ( cfg->GfxRead_Encode_ActivePixel.pm_BitsPerPixel <= 16 )
	{
		datalen = myEnc_ZLib_16( cfg, un, tilenr );
	}
	...
	{
		datalen = myEnc_ZLib_32( cfg, un, tilenr );
	}
	else
	{
		datalen = -1;
