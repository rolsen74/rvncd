...
//	SHELLBUF_PRINTF( "myEnc_ZLib_8 : TileNr #%ld\n", tilenr );
	...
	tmp		= (PTR) & cfg->NetSend_SendBuffer[ 2048 ];
	obuf	= (PTR) & cfg->NetSend_SendBuffer[ sizeof( struct ZLIBHeader ) ];
	...
	for( ry=0 ; ry<h ; ry++ )
	{
		for( rx=0 ; rx<w ; rx++ )
		{
			tmp[pos++] = ibuf[ (yoff + ry) * ti->W + (xoff + rx) ];
		}
	}
	...
	zStream->next_in	= (PTR) tmp;
	zStream->next_out	= (PTR) obuf;
	zStream->avail_in	= pos * sizeof( U8 );
